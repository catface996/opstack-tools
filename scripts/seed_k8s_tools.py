#!/usr/bin/env python3
"""Seed script for creating K8S tools."""
...
async def seed_k8s_tools() -> list[str]:
    """Create K8S tools and return list of created tool IDs."""
    created_tools = []

    async for session in get_session():
        # Get kubernetes category
        category_id = await get_category_id(session, "kubernetes")
        if not category_id:
            print("ERROR: 'kubernetes' category not found. Run seed_categories.py first.")
            return []

        for tool_name in K8S_TOOLS:
            # Get tool definition
            definition = get_tool_definition("k8s", tool_name)
            if not definition:
                print(f"WARNING: No definition found for {tool_name}")
                continue

            # Check if tool already exists
            full_name = definition.get("name", f"k8s_{tool_name}")
            result = await session.execute(
            ...
            # Get script content
            script = get_tool_script("k8s", tool_name)
            if not script:
            ...
            tool = Tool(
                ...
                tags=definition.get("tags", ["kubernetes"]),
...
async def main():
    """Run the seed script."""
    print("Seeding K8S tools...")
    print("-" * 40)

    tool_ids = await seed_k8s_tools()

    print("-" * 40)
    print(f"Created {len(tool_ids)} K8S tools")
    print("\nDone!")
    return tool_ids
