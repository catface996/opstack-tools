#!/usr/bin/env python3
"""Seed script for creating AWS tools."""
...
async def seed_aws_tools() -> list[str]:
    """Create AWS tools and return list of created tool IDs."""
    created_tools = []

    async for session in get_session():
        # Get aws category
        category_id = await get_category_id(session, "aws")
        if not category_id:
            print("ERROR: 'aws' category not found. Run seed_categories.py first.")
            return []

        for tool_name in AWS_TOOLS:
            # Get tool definition
            definition = get_tool_definition("aws", tool_name)
            if not definition:
                print(f"WARNING: No definition found for {tool_name}")
                continue

            # Check if tool already exists
            full_name = definition.get("name", f"aws_{tool_name}")
            result = await session.execute(
            ...
            # Get script content
            script = get_tool_script("aws", tool_name)
            if not script:
...
async def main():
    """Run the seed script."""
    print("Seeding AWS tools...")
    print("-" * 40)

    tool_ids = await seed_aws_tools()

    print("-" * 40)
    print(f"Created {len(tool_ids)} AWS tools")
    print("\nDone!")
    return tool_ids
