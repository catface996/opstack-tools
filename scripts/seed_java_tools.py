#!/usr/bin/env python3
"""Seed script for creating Java tools."""
...
async def seed_java_tools() -> list[str]:
    """Create Java tools and return list of created tool IDs."""
    created_tools = []

    async for session in get_session():
        # Get java category
        category_id = await get_category_id(session, "java")
        if not category_id:
            print("ERROR: 'java' category not found. Run seed_categories.py first.")
            return []

        for tool_name in JAVA_TOOLS:
            # Get tool definition
            definition = get_tool_definition("java", tool_name)
            if not definition:
                print(f"WARNING: No definition found for {tool_name}")
                continue

            # Check if tool already exists
            full_name = definition.get("name", f"java_{tool_name}")
            result = await session.execute(
            ...
            # Get script content
            script = get_tool_script("java", tool_name)
            if not script:
...
async def main():
    """Run the seed script."""
    print("Seeding Java tools...")
    print("-" * 40)

    tool_ids = await seed_java_tools()

    print("-" * 40)
    print(f"Created {len(tool_ids)} Java tools")
    print("\nDone!")
    return tool_ids
